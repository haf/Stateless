...
namespace :env do
  ...
  task :set_dirs do
    ...
    FOLDERS[:s][:out] = File.join(FOLDERS[:src], PROJECTS[:s][:dir], 'bin', CONFIGURATION)
    CLEAN.include(FOLDERS[:s][:out])

    # for tests
    FOLDERS[:s][:test_out] = File.join(FOLDERS[:src], PROJECTS[:s][:test_dir], 'bin', CONFIGURATION)
    FILES[:s][:test] = File.join(FOLDERS[:s][:test_out], "#{PROJECTS[:s][:test_dir]}.dll")
